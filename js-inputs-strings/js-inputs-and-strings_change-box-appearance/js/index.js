...
const myBox = document.querySelector('[data-js="box"]');

colorInput.addEventListener("input", () => {
  myBox.style.background = `hsl(${colorInput.value} 70% 60%)`;
});

radiusInput.addEventListener("input", () => {
  myBox.style.borderRadius = `${radiusInput.value * 2}px`;
});

rotationInput.addEventListener("input", () => {
  myBox.style.transform = `rotate(${rotationInput.value}deg)`;
});
