...
spinButton.addEventListener("click", async () => {
  try {
    ...
    result.setSpinning();
    const value1 = await wheel1.spin();
    const value2 = await wheel2.spin();
    const value3 = await wheel3.spin();

    const values = await Promise.all([value1, value2, value3]);
    console.log(values);
    const numberOfAppearance = getMaxCount(values);
    console.log(numberOfAppearance);
    if (numberOfAppearance == 3) {
      result.setResult(100);
    }
    if (numberOfAppearance == 2) {
      result.setResult(10);
    } else {
      result.setResult(0);
    }
